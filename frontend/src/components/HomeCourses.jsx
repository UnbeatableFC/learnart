...
const HomeCourses = () => {
    ...
    fetch(`${API_BASE}/api/course/public?home=true&limit=8`)
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error(
            text || "Failed to fetch courses from server"
          );
        }
      ...
      .catch((err) => {
        console.error("False to load courses ", err);
  ...
  const handleSetRating = async (e, courseId, rating) => {
    e.stopPropagation();
    if (!isSignedIn) {
      toast("Please Log in to submit a rating...", { icon: "⭐" });
      return;
    }
                  ...
                  >
                    ...
                    <div className={homeCoursesStyles.courseInfo}>
                      ...
                      >
                        ...
                        />
                        <span
                          className={homeCoursesStyles.teacherName}
                        >
                          {course.teacher}
                        </span>
                      </div>
                      <div
                        className={homeCoursesStyles.ratingContainer}
                      >
                        {renderInteractiveStars(course)}
                      </div>
                      <div
                        className={homeCoursesStyles.pricingContainer}
                      >
                        {isFree ? (
                          <span
