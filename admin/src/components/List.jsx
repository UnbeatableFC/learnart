...
const List = () => {
  ...
  const parseDuration = (v) => {
    ...
    if (typeof v === "object") {
      ...
      if (
        "totalMinutes" in v &&
        Number.isFinite(Number(v.totalMinutes))
      ) {
      ...
      if (
        "durationMin" in v &&
        Number.isFinite(Number(v.durationMin))
      ) {
  ...
  const fetchCourses = async () => {
    ...
    try {
      ...
      const mapped = arr.map((c) => {
        ...
        const lecturesArr =
          c.courseLectures || c.lectures || c.contents || [];

        // price could be object or primitive
        const isPriceObject =
          typeof c.price === "object" && c.price !== null;
        ...
        return {
          ...
          instructor:
            c.teacher || c.instructor || "Unknown Instructor",
          image: finalImage,
          rating: typeof c.rating !== "undefined" ? c.rating : 0,
          lectures: Array.isArray(lecturesArr)
            ? lecturesArr.length
            : 0,
  ...
  //   Filter course (by name, instructor, category)
  ...
  const handleRemoveCourse = async (courseId, courseName) => {
    ...
    try {
      ...
      toast.success(
        `"${courseName}" has been removed successfully!`,
        {
          duration: 3000,
      ...
      if (err.response && err.response.status === 401) {
        toast.error(
          "Unauthorized: server requires authentication to delete."
        );
      } else {
  ...
  //   Getting the star rating by user
                      ...
                      <div className={listStyles.courseInfo}>
                        <div className={listStyles.courseTitleRow}>
                          <div className="flex-1 min-w-0">
                            ...
                            <p
                              className={listStyles.courseInstructor}
                            >
                              {course.instructor}
                            </p>
                          </div>
                    ...
                    <div className={listStyles.courseActions}>
                      <div className={listStyles.priceContainer}>
                        <div className="flex items-center gap-1 sm:gap-2">
                          {course.isFree ? (
                            <span className={listStyles.priceFree}>
                              Free
                            </span>
                          ) : (
                            <>
                              <span
                                className={listStyles.priceRegular}
                              >
                                {course.price}
                              </span>
                              {course.originalPrice && (
                                <span
                                  className={listStyles.originalPrice}
                                >
                                  {course.originalPrice}
                                </span>
                              )}
                      ...
                      <div className={listStyles.contentSection}>
                        {(course.courseLectures || []).map(
                          (lecture) => (
                            ...
                            >
                              <div
                                className={listStyles.lectureHeader}
                              >
                                <button
                                  onClick={() =>
                                  ...
                                  className={
                                    listStyles.lectureToggleButton
                                  }
                                >
                                  <div
                                    className={listStyles.lectureInfo}
                                  >
                                    <div className="flex-1 min-w-0">
                                      <h5
                                        className={
                                          listStyles.lectureTitle
                                        }
                                      >
                                        {lecture.title}
                                      </h5>
                              ...
                              {expandedLectures[
                                ...
                                <div
                                  className={
                                    listStyles.expandedLecture
                                  }
                                >
                                  <div
                                    className={listStyles.chapterList}
                                  >
                                    {(lecture.chapters || []).map(
                                      (chapter) => (
                                        <div
                                          ...
                                          className={
                                            listStyles.chapterCard
                                          }
                                        >
                                          <div
                                            className={
                                              listStyles.chapterContent
                                            }
                                          >
                                            <div
                                              className={
                                                listStyles.chapterHeader
                                              }
                                            >
                                              <div
                                                className={
                                                  listStyles.chapterIcon
                                                }
                                              >
                                                <Eye
                                                  className={
