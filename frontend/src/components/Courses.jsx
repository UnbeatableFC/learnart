...
// Show 5 interactive stars (calls onRate())
const RatingStars = ({
  courseId,
  userRating = 0,
  avgRating = 0,
...
const Courses = () => {
  ...
  const submitRatingToServer = async (courseId, ratingValue) => {
    try {
      ...
      const avg =
        data.avgRating ?? data.course?.avgRating ?? data.avg ?? null;
      const total =
        data.totalRatings ??
        data.course?.totalRatings ??
        data.count ??
        null;
          ...
          prev.map((c) =>
            String(c.id) === String(courseId)
              ? {
                  ...c,
                  avgRating:
                    typeof avg === "number" ? avg : c.avgRating,
                  totalRatings:
                    typeof total === "number"
                      ? total
                      : c.totalRatings,
      ...
      toast.error(
        error.message || "Failed to submit rating to the server"
      );
      return false;
    }
  };
  ...
  const handleRating = async (courseId, newRating, e) => {
    ...
    if (!isSignedIn) {
      toast("Please sign in to submit rating", { icon: "⭐" });
      return;
    }
    setRatings((prev) => ({
      ...prev,
                ...
                return (
                  ...
                  >
                    <div className={coursePageStyles.courseCardInner}>
                      <div
                        className={coursePageStyles.courseCardContent}
                      >
                        {/* Image */}
                        <div
                          className={
                        ...
                        <div className={coursePageStyles.courseInfo}>
                          ...
                          >
                            <UserIcon />
                            <span
                              className={coursePageStyles.teacherName}
                            >
                              {course.teacher}
                            </span>
                          </div>
                          ...
                          >
                            <div
                              className={coursePageStyles.ratingStars}
                            >
                              <div
                                className={
