  ...
  const fetchBookings = async (search = "") => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/booking?${q.toString()}`,
        {
          method: "GET",
      ...
      if (data && data.success) {
        const normalized = (data.bookings || []).map((b, idx) => ({
          ...
          studentName:
            b.studentName || b.userName || "Unknown student",
    ...
    <div className={bookingsStyles.pageContainer}>
      <div className={bookingsStyles.contentContainer}>
        <div className={bookingsStyles.headerContainer}>
          <h1 className={bookingsStyles.headerTitle}>
            Course Booking
          </h1>
          <p className={bookingsStyles.headerSubtitle}>
            Manage and view all student courses purchases
          </p>
        </div>
        ...
        >
          ...
          {!loading && error && (
            <div className={bookingsStyles.error}>
              <p>Error : {error}</p>
            </div>
          )}
        </div>
        ...
        <div className={bookingsStyles.bookingsGrid}>
          {!loading &&
            bookings.map((booking) => (
              <div
                key={booking.id}
                className={bookingsStyles.bookingCard}
              >
                <div className={bookingsStyles.studentSection}>
                  <div
                    className={bookingsStyles.studentIconContainer}
                  >
          ...
          <div className={bookingsStyles.emptyState}>
            <div className={bookingsStyles.emptyContainer}>
              ...
              <p className={bookingsStyles.emptyText}>
                No booking matched your search criteria. Try adjusting
                your search terms.
              </p>
            </div>
          </div>
