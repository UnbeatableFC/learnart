...
const stripe = STRIPE_KEY
  ? new Stripe(STRIPE_KEY, { apiVersion: "2025-12-15" })
  : null;
...
// Check for the booking as a user has already enrolled in that course or not
...
export const createBooking = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(
        "DB error saving booking after stripe session:",
        dbErr
      );
      return res.status(500).json({
        success: false,
...
export const getUserBookings = async (req, res) => {
  try {
    ...
    console.error("GetUserBooking:", err);
    return res
      .status(500)
      .json({ success: false, message: "Server error" });
  }
...
export const getStats = async (req, res) => {
  try {
    ...
    const totalRevenue =
      (totalRevenueAgg[0] && totalRevenueAgg.total) || 0;
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

    const bookingLast7Days = await Booking.countDocuments({
