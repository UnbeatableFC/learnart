    ...
    const fetchStats = () =>
      fetch(`${API_BASE}/api/booking/stats`)
        ...
        .then((j) =>
          j.success
            ? j.stats
            : Promise.reject(j.message || "Stats error")
        );
    ...
    const fetchCourses = () =>
      fetch(`${API_BASE}/api/course`)
        ...
        .then((j) =>
          j.success
            ? j.courses
            : Promise.reject(j.message || "Course error")
        );
      ...
      .then(([stats, courses]) => {
        ...
        const mapped = (courses || []).map((c) => {
          ...
          if (c.price && (c.price.sale || c.price.original)) {
            ...
            const orig =
              c.price.original != null
                ? Number(c.price.original)
                : null;
            priceDisplay =
              sale != null
                ? fmtCurrency(sale)
  ...
  return (
    <div className={dashboardStyles.pageContainer}>
      ...
      <div className={dashboardStyles.contentContainer}>
        ...
        {error && (
          <div className={dashboardStyles.errorBanner}>{error}</div>
        )}
          ...
          <div className={dashboardStyles.tableContainer}>
            <table className={dashboardStyles.table}>
              <thead className={dashboardStyles.tableHead}>
                <tr>
                  <th className={dashboardStyles.tableHeader}>
                    Course
                  </th>
                  <th className={dashboardStyles.tableHeader}>
                    Students
                  ...
                  >
                    <td className="px-4 py-3 sm:px-6 sm:py-4">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p
                            className={
                              dashboardStyles.courseInstructor
                            }
                          >
                            {course.instructor}
                          </p>
                    ...
                    <td className={dashboardStyles.studentsCell}>
                      <div className="flex items-center text-gray-700">
                        <span
                          className={dashboardStyles.studentsText}
                        >
                          {course.students}
                        </span>
                      </div>
                    ...
                    <td className="px-4 py-3 sm:px-6 sm:py-4">
                      <div
                        className={dashboardStyles.purchasesContainer}
                      >
                        <ShoppingCart
                          className={dashboardStyles.purchasesIcon}
                        />
                        <span
                          className={dashboardStyles.purchasesText}
                        >
                          {course.purchases}
                        </span>
                      </div>
