...
export const createCourse = async (req, res) => {
  try {
    ...
    // parse price
    const priceParsed =
      parseJSONSafe(body.price) ?? (body.price || {});
    const price = {
      original: toNumber(
        priceParsed.original ?? body["price.original"] ?? 0
      ),
    ...
    // lectures
    let lectures =
      parseJSONSafe(body.lectures) ?? body.lectures ?? [];
...
export const rateCourse = async (req, res) => {
  try {
    ...
    const avgRating =
      totalRatings === 0
        ? 0
        : Number((sum / totalRatings).toFixed(2));
