...
// This is a helper function to normalize lecture/chapter total
const computeCourseTotals = (lectures = []) => {
  const cloned = (Array.isArray(lectures) ? lectures : []).map(
    (lecture) => {
      const lec = {
        ...lecture,
        duration: {
        ...
        chapters: Array.isArray(lecture?.chapters)
          ? [...lecture.chapters]
          : [],
      };
...
}; //This function will calcuclate the total time and override the manual input given by the user.
...
const Add = () => {
  ...
  const [selectedLectureIndex, setSelectedLectureIndex] =
    useState(null);

  // To toggle between the lectures...
  ...
  //   To calculate total minutes, duration + total number of lectures
  ...
  const validateForm = () => {
    ...
    if (
      ...
      Number(formData.totalLectures) <= 0
    ) {
    ...
    if (formData.pricingType === "paid") {
      if (
        ...
        toast.error(
          "Please enter valid original price for paid course"
        );
        return false;
      }
      if (
        !formData.price.sale ||
        parseFloat(formData.price.sale) <= 0
      ) {
  ...
  const addLecture = () => {
    ...
    const lecture = {
      ...
      chapters: (currentLecture.chapters || []).map((ch) => ({
        ...ch,
      })),
    };
  ...
  const removeChapter = (lectureIndex, chapterIndex) => {
    const updated = formData.lectures.map((lec, li) => {
      ...
      return {
        ...lec,
        chapters: (lec.chapters || []).filter(
          (_, ci) => ci !== chapterIndex
        ),
      };
    });
    const computed = computeCourseTotals(updated);
  ...
  const submitToBackend = async () => {
    ...
    try {
      ...
      fd.append(
        "totalDuration",
        JSON.stringify(computed.totalDuration)
      );
      ...
      <div className={addPageStyles.contentContainer}>
        <div className={addPageStyles.headerContainer}>
          <div className={addPageStyles.headerGradient}>
            ...
            <h1 className={addPageStyles.headerTitle}>
              Create New Course
            </h1>
          </div>
          <p className={addPageStyles.headerSubtitle}>
            Craft an exceptional learning experience with our
            intuitive course creation platform
          </p>
        </div>
          ...
          >
            <div className={addPageStyles.cardHeader}>
              <div className={addPageStyles.cardIconContainer}>
                <BookOpenText
                  className={addPageStyles.cardIcon}
                  size={20}
                />
              </div>
              <div>
                <h2 className={addPageStyles.cardTitle}>
                  Course Type
                </h2>
                ...
                <div className={addPageStyles.uploadContainer}>
                  <label className={addPageStyles.uploadLabel}>
                    ...
                    <div className={addPageStyles.uploadBox}>
                      <Upload
                        size={18}
                        className={addPageStyles.uploadIcon}
                      />
          ...
          >
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  ...
                  {formData.lectures.length > 0 ? (
                    ...
                    <p className={addPageStyles.cardSubtitle}>
                      Add lectures and chapters to structure your
                      course
                    </p>
                  )}
                </div>
            ...
            <div className={addPageStyles.lecturesList}>
              {formData.lectures.map((lecture, lectureIndex) => (
                <div
                  key={lecture.id}
                  className={addPageStyles.lectureCard}
                >
                  ...
                  {expandedLectures.includes(lectureIndex) &&
                    lecture.chapters?.length > 0 && (
                      <div
                        className={addPageStyles.chaptersContainer(
                          true
                        )}
                      >
                        {lecture.chapters.map(
                          (chapter, chapterIndex) => (
                            ...
                            >
                              <div
                                className={
                                  addPageStyles.chapterContent
                                }
                              >
                                <div
                                  className={
                                    addPageStyles.chapterInfo
                                  }
                                >
                                  <h4
                                    className={
                                      addPageStyles.chapterTitle
                                    }
                                  >
                                    {chapter.name}
                                  </h4>
                                  ...
                                  <div
                                    className={
                                      addPageStyles.chapterMeta
                                    }
                                  >
                                    <span
                                      className={
                                        addPageStyles.chapterDuration
                                      }
                                    >
                                      <Clock size={12} />{" "}
                                      {formatDuration(
                                ...
                                <button
                                  ...
                                  className={
                                    addPageStyles.chapterDeleteButton
                                  }
                                >
                                  <X size={14} />
                                </button>
          ...
          <div className={addPageStyles.modalOverlay}>
            <div className={addPageStyles.modal}>
              <div className={addPageStyles.modalHeader}>
                <div
                  className={addPageStyles.modalIconContainer(
                    "bg-sky-300"
                  )}
                >
                    ...
                    <div className={addPageStyles.chaptersList}>
                      {currentLecture.chapters.map((chapter) => (
                        ...
                        >
                          <div
                            className={
                              addPageStyles.chapterPreviewTitle
                            }
                          >
                            {chapter.name}
                          </div>
                          <div
                            className={
                              addPageStyles.chapterPreviewDuration
                            }
                          >
